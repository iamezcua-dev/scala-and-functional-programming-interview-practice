...
object ScalaRecap extends App {
  ...
  class CarOfTheFuture extends Car with SelfDriving {
    override def drive(): Unit = println(s"I am driving by myself! Wooohooo! I hope I don't hit any pedestrians ... Who's Asimov?!")
  }

  // case classes are lightweight data structures with boilerplate e.g. equals/hashCode already implemented
  case class Person(name: String, age: Int) {
    def drives(car: Car): Unit = println(s"$name is driving $car") // <-- s-interpolated string
  }
  ...
  // sets: every elements appears once
  ...
  //                                    ^^^^^^ here, the compiler injects the implicit ordering I defined above, as as arguments
